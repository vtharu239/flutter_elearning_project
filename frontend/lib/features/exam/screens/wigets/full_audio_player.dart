...
class FullAudioPlayerWidget extends StatefulWidget {
  final String audioUrl;

  const FullAudioPlayerWidget({super.key, required this.audioUrl});

  @override
  FullAudioPlayerWidgetState createState() => FullAudioPlayerWidgetState();
}

class FullAudioPlayerWidgetState extends State<FullAudioPlayerWidget> {
  ...
  // Thêm danh sách các StreamSubscription để quản lý listener
  ...
  void initState() {
    ...
    // Lưu các listener vào danh sách để hủy sau này
    _subscriptions = [
  ...
  @override
  void dispose() {
    // Hủy tất cả các listener trước khi dispose AudioPlayer
          ...
          Row(
            ...
            children: [
              IconButton(
                ...
                onPressed: () async {
                  if (_isDisposed) return; // Không làm gì nếu đã dispose
                  if (isPlaying) {
      ...
      child: Container(
        ...
        child: Text(
          ...
          style: TextStyle(
            ...
            fontWeight:
                playbackSpeed == speed ? FontWeight.bold : FontWeight.normal,
          ),
        ),
      ),
    );
